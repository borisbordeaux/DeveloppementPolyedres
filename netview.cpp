...
void NetView::setXRotation(int angle)
{
...
void NetView::setYRotation(int angle)
{
    qNormalizeAngle(angle);
    if(angle != m_yRot)
    {
...
void NetView::setZRotation(int angle)
{
    qNormalizeAngle(angle);
    if(angle != m_zRot)
    {
...
void NetView::initializeGL()
{
    ...
    std::cout << "Net View : " << glGetString(GL_VERSION) << std::endl;
    ...
    m_vbo.create();
    m_vbo.bind();
    m_vbo.allocate(m_net.constData(), m_net.count() * sizeof(GLfloat));
...
void NetView::paintGL()
{
    ...
    glDrawArrays(GL_TRIANGLES, 0, m_net.vertexCount());

    m_program->release();
}

void NetView::resizeGL(int w, int h)
{
...
void NetView::mouseMoveEvent(QMouseEvent *event)
{
